...
namespace Cosmoser.PingAnMeetingRequest.Common.ClientService
{
    public class RestXmlClientService : IConferenceHandler
    {
        ...
        public bool Login(ref Model.HandlerSession session)
        {
            try
            {
                ...
                if (status == "200")
                {
                    ...
                    foreach (var item in confType.Split(",".ToCharArray()))
                    {
        ...
        public bool EndMeeting(string conferId, Model.HandlerSession session, out string error)
        {
            error = string.Empty;
            try
            {
                session.AddMessageId();
                string xmlData = string.Format("<?xml version=\"1.0\" encoding=\"utf-8\"?><stopConfer><messageId>{0}</messageId><token>{1}</token><conferId>{2}</conferId><conf_operate_obj>1</conf_operate_obj></stopConfer>", session.MessageId, session.Token, conferId);
                ////logger.Debug(string.Format("DeleteMeeting, xmldata: {0}", xmlData));
                var response = this._client.DoHttpWebRequest(session.BaseUrl + "stopConfer", xmlData);
                ////logger.Debug(string.Format("DeleteMeeting response, xmldata: {0}", response.OuterXml));
                XmlNode root = response.SelectSingleNode("stopConfer");
                string status = root.SelectSingleNode("result").InnerText;
                ...
                else
                {
                    ...
                    //this.ReLogin(session, root.SelectSingleNode("result"));
                    //return false;
                    error = response.SelectSingleNode("stopConfer").SelectSingleNode("result").Attributes["property"].Value;
                    ////logger.Error(string.Format("DeleteMeeting failed, status: {0}, error:{1}", status, response.InnerXml));
                    this.ReLogin(session, response.SelectSingleNode("stopConfer").SelectSingleNode("result"));
                    return false;
                }
            }
            catch (Exception ex)
